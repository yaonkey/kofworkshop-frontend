<template>
  <div class="mt-16 md:mt-0 text-center">
    ...
    <div class="grid sm:grid-cols-2 md:grid-cols-3 mt-16 gap-16">
        <div v-for="item of news" :key="item.id" class="cursor-pointer flex gap-4 items-start" @click="goToProduct(item.id)">
          <div
          ...
          <div class="text-center">
            <h3 class="font-semibold text-lg hover:underline">{{ item.title }}</h3>
          </div>
          <div class="text-center">
            <h4 class="text-lg font-medium text-gray-400">{{ item.price }} руб.</h4>
          </div>
...
const goToProduct = (productNumber: number) => {
  router.push(`/product/${productNumber}`)
}
</script>
